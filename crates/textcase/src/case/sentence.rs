        ...
        match token.kind {
            TokenKind::Word => {
                ...
                token.text = if options.preserve_acronyms && is_all_caps(&original) {
                    original
                } else if options.preserve_mixed_case && is_mixed_case(&original) {
                    original
...
fn recase_word(
    ...
    if mode_is_title(options.mode) {
        if should_keep_lowercase_in_title(profile, lower, is_edge_word) {
